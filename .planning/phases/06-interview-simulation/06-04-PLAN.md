...
<!-- EXECUTOR NOTE: This plan is intentionally kept as a single unit despite its complexity (5 components + orchestration)
     because the components are tightly coupled and splitting would increase cross-plan wiring complexity.
     Recommended build order: Task 1 (supporting components: Timer, Buttons, Reveal, Waveform) FIRST,
     then Task 2 (InterviewSession orchestration + InterviewPage integration) SECOND. -->
    ...
    1. Create `src/components/interview/InterviewSession.tsx`:
       ...
       - **Flow per question** (orchestrated via questionPhase state transitions):
         ...
         c. `responding`:
            ...
            - Show Burmese translation if enabled (uses existing `useLanguage().showBurmese` global toggle from app settings — NOT a new in-session toggle)
       ...
       - **Practice mode quit**:
         ...
         - On confirm: stop recording, call onComplete with current results and endReason='quit'
         - **Claude's discretion**: Don't save partial sessions to Supabase (partial interviews aren't meaningful for trend tracking)
         - NOTE: Do NOT record mastery here — ALL mastery recording is consolidated in Plan 06-05's InterviewResults component on mount (regardless of endReason)
