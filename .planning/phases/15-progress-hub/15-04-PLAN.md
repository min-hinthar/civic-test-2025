...
must_haves:
  truths:
    ...
    - "Badges grouped by category (Study, Test, Streak) with section headers"
...
Purpose: The Achievements tab consolidates the badges and leaderboard from the old SocialHubPage into one premium view. Badges are reorganized by user-friendly categories (Study, Test, Streak) instead of internal category names. Earned badges get visual premium treatment with glow and shimmer effects.
...
- **Category grouping** — create a display-layer mapping (do NOT modify badgeDefinitions.ts):
  ...
  - If a group would be empty (e.g., "Social" has no badges), omit it entirely
...
Run `npx tsc --noEmit` to verify type safety. Confirm the component handles earned, locked, and empty states. Verify the category grouping mapping works correctly with all existing badge definitions.
  ...
  <done>AchievementsTab renders badges grouped by display category (Study, Test, Streak) with earned/locked states, progress bars, glow/shimmer effects, leaderboard with pagination, and empty state.</done>
</task>
...
2. Achievements tab shows badges grouped under Study, Test, Streak headers
