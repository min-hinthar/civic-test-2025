...
<task type="auto">
  ...
  <files>src/lib/ttsCore.ts, src/lib/pwa/syncQueue.ts, src/contexts/OfflineContext.tsx, src/lib/srs/fsrsEngine.ts, src/contexts/SRSContext.tsx</files>
  <action>
**Domain 1: TTS/Audio Investigation**

Read and audit these files in order:
1. `src/lib/ttsCore.ts` — Check Chrome 15s cutoff workaround, Safari cancel errors, Firefox race conditions, Android pause breakage
...
Fix: If `isDue()` has timezone issues, normalize both dates to same timezone. If FSRS params are off, adjust the `w` parameter array per ts-fsrs documentation defaults.
  ...
  <verify>Run `npm run typecheck` and `npm run test:run -- src/lib/ttsCore.test.ts` for TTS. Run any existing sync/SRS tests. Document all findings (verified clean or fixed) in SUMMARY.</verify>
...
<task type="auto">
  <name>Task 2: Investigate leaderboard, PWA, responsive, toast, error boundary, and performance issues</name>
  <files>src/lib/social/index.ts, src/contexts/SocialContext.tsx, src/components/ui/BilingualToast.tsx, src/components/ui/ErrorBoundary.tsx</files>
  <action>
...
Fix 2-3 quick wins. This is a bug fix phase, not a performance overhaul — scope accordingly.
  ...
  <verify>Run `npm run typecheck` and `npm run test:run`. Document all findings per domain (4-9) in SUMMARY — either "verified clean" or describe the fix. No regressions in existing tests.</verify>
  <done>Domains 4-9 (leaderboard, PWA, responsive, toast, error boundary, performance) investigated. Issues fixed or verified clean. All findings documented.</done>
</task>

<task type="auto">
  <name>Task 3: Loading and empty state audit across all views</name>
  <files>src/pages/DashboardPage.tsx, src/pages/StudyGuidePage.tsx, src/pages/HubPage.tsx, src/pages/InterviewPage.tsx, src/pages/MockTestPage.tsx</files>
  <action>
**Loading/Empty State Audit**

Audit all views for proper loading and empty states. Phase 33 implemented these — verify they still work and fix any regressions.

**Dashboard:**
- Does it show skeleton while loading SRS/social data?
- Empty state for new users with no study history?
- Due cards banner showing correctly?

**Study Guide:**
- Loading skeleton while questions load?
- Empty state if category filter returns no results (edge case with search from Plan 05)?

**Hub tabs:**
- History tab: Loading spinner? Empty state for "no sessions yet"?
- Achievements tab: Loading? Empty state for "no badges earned"?
- SRS Deck tab: Loading? Empty state for "no cards in deck"?

**Interview/Mock Test:**
- Loading states during question setup?
- Empty state if no questions available (should not happen with static data, but verify graceful handling)?
- Results screen loading if score calculation is async?

For each view:
1. Read the component and check for loading/empty patterns
2. Verify loading states use skeleton or spinner (not blank screen)
3. Verify empty states have user-friendly message with actionable guidance (e.g., "Start studying to see your progress here")
4. Fix any missing or broken loading/empty states
  </action>
  <verify>Run `npm run typecheck`. Navigate through all major views — confirm no blank screens during load. Confirm empty states render with helpful messages. No regressions.</verify>
  <done>All major views audited for loading/empty states. Missing states added, broken states fixed. All findings documented in SUMMARY.</done>
</task>
