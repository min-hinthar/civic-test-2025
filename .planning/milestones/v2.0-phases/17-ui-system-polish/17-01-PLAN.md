...
<task type="auto">
  ...
  **Simplify `.glass-nav` to gradient overlay only** (so tier classes handle blur/background without conflict):
  - Remove `backdrop-filter`, `-webkit-backdrop-filter`, and `background` from `.glass-nav`
  - Keep `.glass-nav` for ONLY the gradient overlay and box-shadow:
    ```css
    .glass-nav {
      /* Gradient overlay -- tier class handles blur and base background */
      background-image: linear-gradient(
        to bottom,
        hsl(var(--color-surface) / 0.15),
        transparent
      );
      box-shadow: 1px 0 0 0 hsl(var(--color-border) / 0.2);
    }
    ```
  - This allows navigation components (Plan 03) to compose `glass-heavy glass-nav` or `glass-medium glass-nav` without backdrop-filter conflicts
  ...
  **Add overscroll behavior to body:**
  ...
  <done>globals.css has three glass tiers (glass-light, glass-medium, glass-heavy), backward compat .glass-card alias, .glass-nav simplified to gradient overlay + box-shadow only (no backdrop-filter), premium mesh background in both themes, view transitions CSS, and @supports fallback for no-backdrop-filter browsers.</done>
</task>
...
5. `.glass-nav` has NO backdrop-filter (simplified to gradient overlay + box-shadow only)
