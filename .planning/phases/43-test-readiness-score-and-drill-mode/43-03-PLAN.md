...
<task type="auto">
  ...
  <action>
    ...
    - On config start:
      ...
      2. Capture pre-drill readiness: call useReadinessScore() for readiness.score
    ...
    - Results phase:
      - Render DrillResults with results, questions, preDrillMastery, preDrillReadiness
      - Refresh mastery data: call useCategoryMastery.refresh() so post-drill mastery is current
    - "New Drill" action: reset state, go back to config (re-selects weakest -- per user decision, not replay same set)
  ...
  <done>
    /drill route accessible in App Router. DrillConfig shows focus areas, count selector (5/10/20), estimated time, and Start Drill button. DrillPage manages config -> session -> results flow. DrillBadge distinguishes drill mode from practice. PracticeSession reused for quiz flow. Build succeeds.
  </done>
</task>
