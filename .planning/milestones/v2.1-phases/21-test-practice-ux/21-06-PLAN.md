...
- Check handler calls `hapticLight()` for correct, `hapticDouble()` for incorrect (TPUX-07 wiring)
- Check handler calls existing `playCorrect()`/`playIncorrect()` sound effects on check
