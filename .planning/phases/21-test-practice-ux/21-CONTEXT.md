...
Replace auto-advance quiz flow with an explicit Check/Continue flow (Duolingo-style) in both Mock Test and Practice modes. Includes feedback panel, segmented progress bar, redesigned timer, keyboard navigation, redesigned results screen, and skip/review mechanics. Interview simulation UX is NOT in scope.
...
- "Duolingo-style Check/Continue flow" — explicit reference for the overall UX pattern
...
- Celebration should be multi-sensory: confetti + glow badge + sound effect for passing scores
...
- Interview simulation UX overhaul — separate phase (Interview has its own distinct UX with microphone, grading, conversational flow)
