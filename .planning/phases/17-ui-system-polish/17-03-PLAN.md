...
<task type="auto">
  ...
  **Sidebar.tsx:**
  ...
  - Keep `glass-nav` for the gradient background-image overlay (it provides the top-to-bottom gradient)
  ...
  Note: The `glass-nav` class in globals.css sets backdrop-filter. With `glass-heavy` also setting it, they'll conflict. So update `glass-nav` in globals.css (in the previous plan's work) to NOT set backdrop-filter and instead only provide the gradient overlay and box-shadow. Or, more cleanly: remove `backdrop-filter` and `background` from `.glass-nav` class and let the tier class handle those. Keep `.glass-nav` for just the `background-image` gradient overlay and `box-shadow`. This means updating globals.css:
  ...
  The `box-shadow` for `.glass-nav` can stay as-is since it provides the right-edge shadow for the sidebar.
