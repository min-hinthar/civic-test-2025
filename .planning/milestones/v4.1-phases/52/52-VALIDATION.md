...
|--------|----------|-----------|-------------------|-------------|--------|
| TEST-03 | Auth login -> dashboard | E2E | `npx playwright test e2e/auth-dashboard.spec.ts` | No W0 | pending |
| TEST-04 | Mock test lifecycle | E2E | `npx playwright test e2e/mock-test.spec.ts` | No W0 | pending |
| TEST-05 | Practice session | E2E | `npx playwright test e2e/practice.spec.ts` | No W0 | pending |
| TEST-06 | Flashcard sort | E2E | `npx playwright test e2e/flashcard-sort.spec.ts` | No W0 | pending |
| TEST-07 | Offline -> online sync | E2E | `npx playwright test e2e/offline-sync.spec.ts` | No W0 | pending |
| TEST-08 | Interview session | E2E | `npx playwright test e2e/interview.spec.ts` | No W0 | pending |
| TEST-09 | SW update flow | E2E | `npx playwright test e2e/sw-update.spec.ts --project=chromium-sw` | No W0 | pending |
| A11Y-01 | WCAG 2.2 scans (4 pages) | E2E | `npx playwright test e2e/wcag-scan.spec.ts` | No W0 | pending |
| A11Y-02 | vitest-axe expansion | Unit | `pnpm test:run src/__tests__/a11y/` | Partial (2 exist) | pending |
| A11Y-03 | Touch target 44px audit | E2E + fixes | `npx playwright test e2e/touch-targets.spec.ts` | No W0 | pending |
...
- [ ] `e2e/fixtures/index.ts` — combined test export (test.extend composition with authedPage, makeAxeBuilder, and reducedMotion page override)
...
- [ ] `playwright.config.ts` chromium-sw project: `serviceWorkers: 'allow'`, `testMatch: ['**/sw-update.spec.ts']`

Note: AxeBuilder configuration is consolidated into `e2e/fixtures/index.ts` (not a separate `axe.ts` file). The `makeAxeBuilder` fixture in index.ts provides the pre-configured AxeBuilder with WCAG 2.2 AA tags and glass element exclusions.
