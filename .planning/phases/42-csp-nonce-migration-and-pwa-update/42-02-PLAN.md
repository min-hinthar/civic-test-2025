...
must_haves:
  truths:
    ...
    - "Automated regression tests protect the CSP nonce implementation from future breakage"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    10. Verify `src/contexts/OfflineContext.tsx` still functions correctly with the new SW caching strategy -- confirm it imports and uses the expected navigator.onLine / online/offline event listeners (no code change needed, just verify compatibility)
    11. Verify `src/components/pwa/PWAOnboardingFlow.tsx` still works with App Router routes -- confirm install prompt handling and offline fallback references are intact (no code change needed, just verify compatibility)
  ...
  <verify>
    ...
    <manual>Build succeeds. All pages render. No CSP violations in browser. SW registers in production build. OfflineContext.tsx offline detection and PWAOnboardingFlow.tsx install prompt both function correctly on App Router routes.</manual>
  </verify>
  <done>Full test suite passes. Build succeeds. SW generates correctly. No HASH_REDIRECT_SCRIPT references remain. TypeScript compiles cleanly. OfflineContext and PWAOnboardingFlow verified compatible. Phase 42 requirements MIGR-09 and MIGR-10 are met.</done>
</task>
...
7. OfflineContext.tsx and PWAOnboardingFlow.tsx verified compatible with new SW caching strategy
</verification>
...
- OfflineContext and PWAOnboardingFlow confirmed working with App Router
