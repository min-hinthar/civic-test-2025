...
<task type="auto">
  ...
  <files>src/contexts/OfflineContext.tsx, src/pages/TestPage.tsx, src/hooks/useSyncQueue.ts, src/lib/mastery/fsrsEngine.ts</files>
  <action>
...
2. `useSyncQueue.ts` (~line 87): sync failure toast -> change `variant: 'destructive'` to `variant: 'warning'`
3. `TestPage.tsx` (~lines 240, 283): test-related errors (not data loss) -> change to `variant: 'warning'`

**NOTE:** `AppNavigation.tsx` locked navigation toast reclassification is deferred to Plan 09-06 (Wave 3) which does a full AppNavigation overhaul — it will handle the variant: 'destructive' -> 'warning' change there alongside the Duolingo styling refresh. This avoids a file ownership conflict with Plan 09-03 (Wave 2) which also modifies AppNavigation.
...
- NOTE: AppNavigation toast reclassification handled in Plan 09-06
</verification>
