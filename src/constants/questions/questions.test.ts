...
describe('Question Bank Validation', () => {
  ...
  it('all questions belong to valid categories', () => {
    ...
    for (const q of allQuestions) {
      expect(validCategories, `${q.id} has invalid category: "${q.category}"`).toContain(
        q.category
      );
    }
  });
  ...
  describe('Dynamic Questions', () => {
    ...
    it('dynamic questions have valid type', () => {
      ...
      for (const q of dynamicQs) {
        expect(['time', 'state'], `${q.id} has invalid dynamic type: ${q.dynamic!.type}`).toContain(
          q.dynamic!.type
        );
      }
    });
  ...
  describe('Category Distribution', () => {
    it('has correct category counts', () => {
      ...
      expect(counts.get('Recent American History and Other Important Historical Information')).toBe(
        12
      );
